...
laser.style.display = settings.photoMode === "dot" ? "block" : "none";
      ...
      function renderSettingsPanel() {
        ...
        // НОВАЯ НАСТРОЙКА: РЕЖИМ СЪЕМКИ
        const photoModeSetting = document.createElement("div");
        photoModeSetting.style.cssText = "display:flex; justify-content:space-between; align-items:center; margin-top:12px;";
        
        const photoModeLabel = document.createElement("span");
        photoModeLabel.textContent = "Режим съемки";
        photoModeLabel.style.cssText = "color:#fff; font-weight:bold;";
        
        const photoModeSelect = document.createElement("select");
        photoModeSelect.style.cssText = "background:#333; color:#fff; border:1px solid #500; padding:6px; border-radius:4px;";
        
        const optionDot = document.createElement("option");
        optionDot.value = "dot";
        optionDot.textContent = "Точка (сразу фото)";
        
        const optionDraw = document.createElement("option");
        optionDraw.value = "draw";
        optionDraw.textContent = "Рисование после фото";
        
        photoModeSelect.appendChild(optionDot);
        photoModeSelect.appendChild(optionDraw);
